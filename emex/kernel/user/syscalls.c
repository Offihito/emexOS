...
extern u64 user_r14;
extern u64 user_r15;
extern void resume_parent_sysret(u64 rip, u64 rsp, u64 r11, u64 cr3, u64 rbx,
                                 u64 rbp, u64 r12, u64 r13, u64 r14, u64 r15);
...
static u32 ansi_code_to_color(int code) {
  switch (code) {
  case 0:
    return 0xFFFFFFFF; // reset (white)
  case 30:
    return 0xFF111111; // black
  case 31:
  ...
  case 91:
    return 0xFFFF8888; // bright red
  case 92:
    return 0xFF88FF88; // bright green
  case 93:
    return 0xFFFFFF88; // bright yellow
  case 94:
    return 0xFF8888FF; // bright blue
  case 95:
    return 0xFFFF88FF; // bright magenta
  case 96:
    return 0xFF88FFFF; // bright cyan
  case 97:
    return 0xFFFFFFFF; // bright white
  default:
    return 0xFFFFFFFF;
  }
}
...
u64 scall_exit(ulime_proc_t *proc, u64 exit_code, u64 arg2, u64 arg3) {
      ...
      for (int i = 0; i < mt->task_count; i++) {
        if (mt->tasks[i].proc == parent) {
          ...
          if (mt->tasks[i].kstack)
            gdt_set_kernel_stack(((u64)mt->tasks[i].kstack + MT_KSTACK_SIZE) &
                                 ~0xFULL);
          break;
        }
      }
  ...
  if (mt) {
    for (int i = 0; i < mt->task_count; i++) {
      ...
      if (!t->valid || !t->proc)
        continue;
      if (t->proc->state == PROC_ZOMBIE || t->proc->state == PROC_BLOCKED)
        continue;
      mt->current_idx = i;
      ...
      if (t->user_ctx.saved) {
        resume_parent_sysret(t->user_ctx.rip, t->user_ctx.rsp,
                             t->user_ctx.rflags, t->proc->pml4_phys,
                             t->user_ctx.rbx, t->user_ctx.rbp, t->user_ctx.r12,
                             t->user_ctx.r13, t->user_ctx.r14, t->user_ctx.r15);
      } else {
...
static void setup_argv_on_stack(ulime_proc_t *new_proc, char **user_argv,
                                int argc) {
  if (argc <= 0 || !user_argv)
    return;
...
u64 scall_execve(ulime_proc_t *proc, u64 path_ptr, u64 argv_ptr, u64 arg3) {
  ...
  int is_elf =
      (path_len > 4 && path[path_len - 4] == '.' && path[path_len - 3] == 'e' &&
       path[path_len - 2] == 'l' && path[path_len - 1] == 'f');
...
u64 scall_read(ulime_proc_t *proc, u64 fd, u64 buf, u64 count) {
  ...
  u64 result = (u64)fs_read((int)fd, (void *)buf, (size_t)count);
  if (cur) {
...
u64 scall_getdents(ulime_proc_t *proc, u64 path_ptr, u64 buf_ptr,
                   u64 max_entries) {
  (void)proc;
  if (!is_valid_user_ptr(path_ptr) || !is_valid_user_ptr(buf_ptr))
    return (u64)-1;
  int n = fs_listdir((const char *)path_ptr, (_emx_kdirent_t *)buf_ptr,
                     (int)max_entries);
...
u64 scall_ioctl(ulime_proc_t *proc, u64 fd, u64 request, u64 arg_ptr) {
  ...
  if (fd <= 2) {
    switch ((int)request) {
    case 0:
      tty0_set_echo_mode(0);
      return 0; // TTY_ECHO
    case 1:
      tty0_set_echo_mode(1);
      return 0; // TTY_NOECHO
    case 2:
      tty0_set_echo_mode(2);
      return 0; // TTY_MASKECHO
    case 3:
      tty0_set_echo_mode(3);
      return 0; // TTY_RAW
    default:
      return (u64)-1;
    }
...
u64 scall_fork(ulime_proc_t *proc, u64 arg1, u64 arg2, u64 arg3) {
  ...
  ulime_proc_t *child =
      proc_create_proc(proc_mgr, proc->name, proc->entry_point, proc->priority);
  if (!child)
    return (u64)-1;
  ...
  memcpy((void *)(child->phys_heap + hhdm), (void *)(proc->phys_heap + hhdm),
         proc->heap_size);
  memcpy((void *)(child->phys_stack + hhdm), (void *)(proc->phys_stack + hhdm),
         proc->stack_size);
  ...
  if (proc->vma_base != 0) {
    for (u64 p = 0; p < heap_pages; p++) {
      ...
      u64 child_phys = child->phys_heap + (p * 0x1000);
      paging_map_page_proc(g_ulime->hpr, child->pml4_phys, orig_virt,
                           child_phys, heap_flags);
    }
  }
...
u64 scall_getcwd(ulime_proc_t *proc, u64 buf_ptr, u64 size, u64 arg3) {
  ...
  if (!is_valid_user_ptr_range(buf_ptr, size))
    return 0;
...
u64 scall_waitpid(ulime_proc_t *proc, u64 pid, u64 arg2, u64 arg3) {
  ...
  if (mt) {
    for (int i = 0; i < mt->task_count; i++) {
      ...
      if (t->kstack)
        gdt_set_kernel_stack(((u64)t->kstack + MT_KSTACK_SIZE) & ~0xFULL);
      printf("[SYSCALL] waitpid: '%s' waiting for pid=%llu, running '%s'\n",
...
u64 scall_sysinfo(ulime_proc_t *proc, u64 info_addr, u64 a1, u64 a2) {
  ...
  if (!is_valid_user_ptr_range(info_addr, sizeof(struct sysinfo_t)))
    return (u64)-1;
