...
static int mt_find_next(mt_t *mt, int from_idx)
{
    ...
    do
    {
        ...
        if (t->valid && t->proc &&
            (
            	t->proc->state == PROC_READY         ||
             	t->proc->state == PROC_CREATED
            )
        ) { return i;}
    ...
    // fallback: try the start index itself
...
void mt_start(mt_t *mt)
{
    ...
    // set the quantum for this task based on its priority
    if (mt->sched)
    {
        mt->sched->quantum     = scheduler_quantum_for_priority(t->proc->priority);
        mt->sched->ticks       = 0;
    }
    ...
    printf("[MT] starting first task: idx=%d pid=%llu rip=0x%llx rsp=0x%llx quantum=%llu\n",
           idx,
           (unsigned long long)t->proc->pid,
           (unsigned long long)user_rip,
           (unsigned long long)user_rsp,
           mt->sched ? (unsigned long long)mt->sched->quantum : 0ULL
    );
...
void mt_preempt(mt_t *mt, cpu_state_t *state)
{
    ...
    if (mt->current_idx >= 0)
    {
        ...
        {
            if (mt->sched) mt->sched->ticks = mt->sched->quantum; // expire quantum now
        }
    }
    ...
    if (mt->sched) {
        new_idx = scheduler_pick_next(mt->sched, mt, old_idx);
    } else {
        // fallback plain round robin if no scheduler attached
        new_idx = mt_find_next(mt, old_idx);
    }
    ...
    {
        // first time this task runs, set up a clean initial state
    ...
    // update quantum for the new task based on its priority
    if (mt->sched) {
        mt->sched->quantum = scheduler_quantum_for_priority(next->proc->priority);
        mt->sched->last_idx = new_idx;
    }
